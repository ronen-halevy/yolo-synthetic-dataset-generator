    ...
    def get_dataset(filenames, batch_size):
        dataset = (
            tf.data.TFRecordDataset(filenames, num_parallel_reads=tf.data.AUTOTUNE)
                .map(ReadTfrecordsShapes.parse_tfrecord_fn, num_parallel_calls=tf.data.AUTOTUNE)
                # .map(ReadTfrecordsShapes.prepare_sample, num_parallel_calls=tf.data.AUTOTUNE)
